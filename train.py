...
def atest_rnn_dec_epoch(epoch, graphs_test, args, encoder, rnn, output, test_batch_size=16):
    ...
    graph_embedding = (graph_embedding - graph_embedding.mean(dim=1, keepdim=True)) / (
                graph_embedding.std(dim=1, keepdim=True) + 1e-8)
            ...
            for sample_time in range(1, 4):
                ...
                while len(G_pred) < args.test_total_size:
                    ...
                    if 'GraphRNN_VAE' in args.note:
                        ...
                        G_pred_step = test_rnn_with_attention_epoch(epoch, args, rnn, output,
    ...
    if epoch % args.epochs_test == 0 and epoch >= args.epochs_test_start:
        ...
        for sample_time in range(1, 4):
            ...
            while len(G_pred) < args.test_total_size:
                ...
                G_pred_step = test_rnn_dec_epoch(epoch, dataset_test, args, encoder, rnn, output,
                                                 test_batch_size=args.test_batch_size)
                G_pred.extend(G_pred_step)
            # Save graphs
