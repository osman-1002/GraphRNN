...
class Graph_sequence_sampler_pytorch(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            # torch.tensor(edge_index, dtype=torch.long),  # Assuming it contains indices
            'batch': batch.clone().detach(),  # torch.tensor(batch, dtype=torch.long),  # Assuming it contains indices
            'edge_seq': edge_seq.clone().detach()
            # torch.tensor(edge_seq, dtype=torch.float32)  # Adjust dtype based on your data
